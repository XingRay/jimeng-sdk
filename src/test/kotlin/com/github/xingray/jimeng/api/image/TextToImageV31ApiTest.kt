...
import com.github.xingray.jimeng.model.image.TextToImageV31Request
...
class TextToImageV31ApiTest : BaseApiTest() {

    private fun createRequest() = TextToImageV31Request(prompt = "雪山下的小木屋", width = 512, height = 512)

    @Test
    fun testSubmit(): Unit = runBlocking {
        val resp = client.textToImageV31.submit(credential, createRequest())
    ...
    fun testQueryUrl(): Unit = runBlocking {
        val submitResp = client.textToImageV31.submit(credential, createRequest())
        assertTrue(submitResp.isSuccess, "submit failed: ${submitResp.message}")
        val taskId = submitResp.data!!.taskId

        var attempts = 0
        var resp = client.textToImageV31.queryUrl(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            ...
            delay(3000L)
            resp = client.textToImageV31.queryUrl(credential, taskId)
    ...
    fun testQueryBase64(): Unit = runBlocking {
        ...
        var attempts = 0
        var resp = client.textToImageV31.queryBase64(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            check(++attempts < 200) { "polling timed out" }
            delay(3000L)
            resp = client.textToImageV31.queryBase64(credential, taskId)
    ...
    fun testSubmitAndWaitForUrl(): Unit = runBlocking {
        val resp = client.textToImageV31.submitAndWaitForUrl(credential, createRequest())
    ...
    fun testSubmitAndWaitForBase64(): Unit = runBlocking {
        val resp = client.textToImageV31.submitAndWaitForBase64(credential, createRequest())
