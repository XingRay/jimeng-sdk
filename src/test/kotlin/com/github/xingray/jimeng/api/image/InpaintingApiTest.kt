...
import com.github.xingray.jimeng.model.image.InpaintingRequest
...
class InpaintingApiTest : BaseApiTest() {

    private fun createRequest() = InpaintingRequest(prompt = "添加一朵花")

    @Test
    fun testSubmit(): Unit = runBlocking {
        val resp = client.inpainting.submit(credential, createRequest())
    ...
    fun testQueryUrl(): Unit = runBlocking {
        val submitResp = client.inpainting.submit(credential, createRequest())
        assertTrue(submitResp.isSuccess, "submit failed: ${submitResp.message}")
        val taskId = submitResp.data!!.taskId

        var attempts = 0
        var resp = client.inpainting.queryUrl(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            ...
            delay(3000L)
            resp = client.inpainting.queryUrl(credential, taskId)
    ...
    fun testQueryBase64(): Unit = runBlocking {
        ...
        var attempts = 0
        var resp = client.inpainting.queryBase64(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            check(++attempts < 200) { "polling timed out" }
            delay(3000L)
            resp = client.inpainting.queryBase64(credential, taskId)
    ...
    fun testSubmitAndWaitForUrl(): Unit = runBlocking {
        val resp = client.inpainting.submitAndWaitForUrl(credential, createRequest())
    ...
    fun testSubmitAndWaitForBase64(): Unit = runBlocking {
        val resp = client.inpainting.submitAndWaitForBase64(credential, createRequest())
