...
import com.github.xingray.jimeng.model.image.PortraitPhotoRequest
...
class PortraitPhotoApiTest : BaseApiTest() {

    private fun createRequest() = PortraitPhotoRequest()

    @Test
    fun testSubmit(): Unit = runBlocking {
        val resp = client.portraitPhoto.submit(credential, createRequest())
    ...
    fun testQueryUrl(): Unit = runBlocking {
        val submitResp = client.portraitPhoto.submit(credential, createRequest())
        assertTrue(submitResp.isSuccess, "submit failed: ${submitResp.message}")
        val taskId = submitResp.data!!.taskId

        var attempts = 0
        var resp = client.portraitPhoto.queryUrl(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            ...
            delay(3000L)
            resp = client.portraitPhoto.queryUrl(credential, taskId)
    ...
    fun testQueryBase64(): Unit = runBlocking {
        ...
        var attempts = 0
        var resp = client.portraitPhoto.queryBase64(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            check(++attempts < 200) { "polling timed out" }
            delay(3000L)
            resp = client.portraitPhoto.queryBase64(credential, taskId)
    ...
    fun testSubmitAndWaitForUrl(): Unit = runBlocking {
        val resp = client.portraitPhoto.submitAndWaitForUrl(credential, createRequest())
    ...
    fun testSubmitAndWaitForBase64(): Unit = runBlocking {
        val resp = client.portraitPhoto.submitAndWaitForBase64(credential, createRequest())
