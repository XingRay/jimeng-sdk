...
import com.github.xingray.jimeng.model.image.ImageToImageSmartRefRequest
...
class ImageToImageSmartRefApiTest : BaseApiTest() {

    private fun createRequest() = ImageToImageSmartRefRequest(prompt = "风格化处理")

    @Test
    fun testSubmit(): Unit = runBlocking {
        val resp = client.imageToImageSmartRef.submit(credential, createRequest())
    ...
    fun testQueryUrl(): Unit = runBlocking {
        val submitResp = client.imageToImageSmartRef.submit(credential, createRequest())
        assertTrue(submitResp.isSuccess, "submit failed: ${submitResp.message}")
        val taskId = submitResp.data!!.taskId

        var attempts = 0
        var resp = client.imageToImageSmartRef.queryUrl(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            ...
            delay(3000L)
            resp = client.imageToImageSmartRef.queryUrl(credential, taskId)
    ...
    fun testQueryBase64(): Unit = runBlocking {
        ...
        var attempts = 0
        var resp = client.imageToImageSmartRef.queryBase64(credential, taskId)
        while (resp.isSuccess && resp.data?.status.let { it == TaskStatus.IN_QUEUE || it == TaskStatus.GENERATING || it == TaskStatus.PROCESSING }) {
            check(++attempts < 200) { "polling timed out" }
            delay(3000L)
            resp = client.imageToImageSmartRef.queryBase64(credential, taskId)
    ...
    fun testSubmitAndWaitForUrl(): Unit = runBlocking {
        val resp = client.imageToImageSmartRef.submitAndWaitForUrl(credential, createRequest())
    ...
    fun testSubmitAndWaitForBase64(): Unit = runBlocking {
        val resp = client.imageToImageSmartRef.submitAndWaitForBase64(credential, createRequest())
