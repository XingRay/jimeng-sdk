 ...
 * 即梦互动编辑 API。
 *
 * 支持图像局部重绘和修复，提交任务后通过URL或Base64两种方式获取结果。
 *
...
class InpaintingApi(
    ...
    private val reqKey = ReqKey.INPAINTING

    /** 提交互动编辑任务 */
    suspend fun submit(credential: Credential, request: InpaintingRequest): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: InpaintingRequest,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
