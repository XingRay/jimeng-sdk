 ...
 * 图片变体智能体 API。
 *
 * 生成图片变体，支持文字和贴纸叠加，提交任务后通过URL或Base64两种方式获取结果。
 *
 * @param client 火山引擎API客户端
 * @param pollIntervalMs 轮询间隔时间（毫秒）
 * @param maxPollAttempts 最大轮询次数
 */
class ImageVariationAgentApi(
    private val client: VolcApiClient,
    private val pollIntervalMs: Long = 3000L,
    private val maxPollAttempts: Int = 200
) {
    private val reqKey = ReqKey.IMAGE_VARIATION_AGENT

    /** 提交图片变体任务 */
    suspend fun submit(credential: Credential, request: ImageVariationAgentRequest): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: ImageVariationAgentRequest,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
