 ...
 * 素材抽取-POD API。
 *
 * 提取图像素材用于印刷定制（Print on Demand），支持提交任务后通过URL或Base64两种方式获取结果。
 *
 * @param client 火山引擎API客户端
 * @param pollIntervalMs 轮询间隔时间（毫秒）
 * @param maxPollAttempts 最大轮询次数
 */
class MaterialExtractionPodApi(
    private val client: VolcApiClient,
    private val pollIntervalMs: Long = 3000L,
    private val maxPollAttempts: Int = 200
) {
    private val reqKey = ReqKey.MATERIAL_EXTRACTION_POD

    /** 提交素材抽取-POD任务 */
    suspend fun submit(credential: Credential, request: MaterialExtractionPodRequest): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: MaterialExtractionPodRequest,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
