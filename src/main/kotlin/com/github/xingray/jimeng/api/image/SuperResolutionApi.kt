 ...
 * 智能高清 API。
 *
 * 图像超分辨率放大，提交任务后通过URL或Base64两种方式获取结果。
 *
...
class SuperResolutionApi(
    ...
    private val reqKey = ReqKey.SUPER_RESOLUTION

    /** 提交智能高清任务 */
    suspend fun submit(credential: Credential, request: SuperResolutionRequest): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: SuperResolutionRequest,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
