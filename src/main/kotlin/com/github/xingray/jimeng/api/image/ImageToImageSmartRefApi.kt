 ...
 * 即梦图生图3.0 智能参考 API。
 *
 * 根据参考图片和文本描述生成相似风格图像，支持提交任务后通过URL或Base64两种方式获取结果。
 *
 * @param client 火山引擎API客户端
 * @param pollIntervalMs 轮询间隔时间（毫秒）
 * @param maxPollAttempts 最大轮询次数
 */
class ImageToImageSmartRefApi(
    private val client: VolcApiClient,
    private val pollIntervalMs: Long = 3000L,
    private val maxPollAttempts: Int = 200
) {
    private val reqKey = ReqKey.IMAGE_TO_IMAGE_SMART_REF_V31

    /** 提交图生图智能参考任务 */
    suspend fun submit(credential: Credential, request: ImageToImageSmartRefRequest): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: ImageToImageSmartRefRequest,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
