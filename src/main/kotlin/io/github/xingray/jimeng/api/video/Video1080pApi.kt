...
import io.github.xingray.jimeng.model.video.Video1080pI2VFirstRequest
import io.github.xingray.jimeng.model.video.Video1080pI2VFirstTailRequest
import io.github.xingray.jimeng.model.video.Video1080pT2VRequest
import kotlinx.coroutines.delay

/**
 * 即梦视频3.0 1080P API。
 *
 * 高分辨率视频生成，支持文生视频和图生视频（首帧/首尾帧），生成1080P分辨率视频。
 *
...
class Video1080pApi(
    ...
    /** 提交1080P文生视频任务 */
    suspend fun submitT2V(credential: Credential, request: Video1080pT2VRequest): ApiResponse<SubmitTaskData> {
        return client.post(credential, JimengConstants.ACTION_SUBMIT_TASK, request)
    }

    /** 提交1080P图生视频任务（首帧模式） */
    suspend fun submitI2VFirst(credential: Credential, request: Video1080pI2VFirstRequest): ApiResponse<SubmitTaskData> {
        return client.post(credential, JimengConstants.ACTION_SUBMIT_TASK, request)
    }

    /** 提交1080P图生视频任务（首尾帧模式） */
    suspend fun submitI2VFirstTail(credential: Credential, request: Video1080pI2VFirstTailRequest): ApiResponse<SubmitTaskData> {
        return client.post(credential, JimengConstants.ACTION_SUBMIT_TASK, request)
    }
    ...
    /** 提交文生视频任务并轮询等待结果 */
    suspend fun submitAndWaitT2V(credential: Credential, request: Video1080pT2VRequest, aigcMeta: AigcMeta? = null): ApiResponse<VideoResult> {
        ...
        repeat(maxPollAttempts) {
            val result = query(credential, ReqKey.VIDEO_T2V_1080P_V30, taskId, aigcMeta)
    ...
    /** 提交图生视频（首帧）任务并轮询等待结果 */
    suspend fun submitAndWaitI2VFirst(credential: Credential, request: Video1080pI2VFirstRequest, aigcMeta: AigcMeta? = null): ApiResponse<VideoResult> {
        ...
        val taskId = submitResp.data.taskId
        repeat(maxPollAttempts) {
            val result = query(credential, ReqKey.VIDEO_I2V_FIRST_1080P_V30, taskId, aigcMeta)
            val status = result.data?.status
            when {
                !result.isSuccess -> return result
                status == TaskStatus.DONE || status == TaskStatus.NOT_FOUND || status == TaskStatus.EXPIRED -> return result
    ...
    suspend fun submitAndWaitI2VFirstTail(credential: Credential, request: Video1080pI2VFirstTailRequest, aigcMeta: AigcMeta? = null): ApiResponse<VideoResult> {
        ...
        val taskId = submitResp.data.taskId
        repeat(maxPollAttempts) {
            val result = query(credential, ReqKey.VIDEO_I2V_FIRST_TAIL_1080P_V30, taskId, aigcMeta)
            val status = result.data?.status
            when {
                !result.isSuccess -> return result
                status == TaskStatus.DONE || status == TaskStatus.NOT_FOUND || status == TaskStatus.EXPIRED -> return result
