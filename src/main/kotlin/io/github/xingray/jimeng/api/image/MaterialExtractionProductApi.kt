...
import io.github.xingray.jimeng.model.image.MaterialExtractionProductRequest
import kotlinx.coroutines.delay

/**
 * 素材抽取-商品图 API。
 *
 * 提取商品素材生成产品图，支持提交任务后通过URL或Base64两种方式获取结果。
 *
 * @param client 火山引擎API客户端
 * @param pollIntervalMs 轮询间隔时间（毫秒）
 * @param maxPollAttempts 最大轮询次数
 */
class MaterialExtractionProductApi(
    private val client: VolcApiClient,
    private val pollIntervalMs: Long = 3000L,
    private val maxPollAttempts: Int = 200
) {
    private val reqKey = ReqKey.MATERIAL_EXTRACTION_PRODUCT

    /** 提交素材抽取-商品图任务 */
    suspend fun submit(credential: Credential, request: MaterialExtractionProductRequest): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: MaterialExtractionProductRequest,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
