...
import io.github.xingray.jimeng.model.image.ImageGenerationV40Request
import kotlinx.coroutines.delay

/**
 * 即梦图像生成4.0 API。
 *
 * 最新一代图像生成模型，支持提交任务后通过URL或Base64两种方式获取结果。
 *
 * @param client 火山引擎API客户端
 * @param pollIntervalMs 轮询间隔时间（毫秒）
 * @param maxPollAttempts 最大轮询次数
 */
class ImageGenerationV40Api(
    private val client: VolcApiClient,
    private val pollIntervalMs: Long = 3000L,
    private val maxPollAttempts: Int = 200
) {
    private val reqKey = ReqKey.TEXT_TO_IMAGE_V40

    /** 提交图像生成4.0任务 */
    suspend fun submit(credential: Credential, request: ImageGenerationV40Request): ApiResponse<SubmitTaskData> {
    ...
    suspend fun submitAndWaitForUrl(
        credential: Credential, request: ImageGenerationV40Request,
        logoInfo: LogoInfo? = null, aigcMeta: AigcMeta? = null
    ): ApiResponse<ImageUrlResult> {
